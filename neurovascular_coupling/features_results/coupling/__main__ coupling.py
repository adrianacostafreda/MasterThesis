...
def coupling(data_eeg, data_fnirs):
    ...
    for (array_eeg, array_fnirs) in zip(data_eeg, data_fnirs):
        ...
        shifted_eeg_arrays = []
        reduced_fnirs_arrays = []

        for shift in range(1, num_shifts+1):
            shift_amount = int(shift * increment)

            #Shift EEG array to the right 
            if shift_amount < mean_channels_eeg.shape[0]:
                shifted_eeg = mean_channels_eeg[shift_amount:]
            else:
                shifted_eeg = np.array([])
        
            if shift_amount < mean_channels_fnirs.shape[0]:
                reduced_fnirs = mean_channels_fnirs[:-shift_amount]
            else:
                reduced_fnirs = np.array([])
            
            # Ensure both arrays have compatible shapes
            if shifted_eeg.shape == reduced_fnirs.shape:
                shifted_eeg_arrays.append(shifted_eeg)
                reduced_fnirs_arrays.append(reduced_fnirs)
            else:
                # In case of dimension mismatch, adjust the lengths to match
                min_length = min(shifted_eeg.shape[1], reduced_fnirs.shape[0])
                if min_length > 0:
                    shifted_eeg_arrays.append(shifted_eeg[:, :min_length, :])
                    reduced_fnirs_arrays.append(reduced_fnirs[:min_length, :, :])
                else:
                    # Handle the case where resulting arrays would be empty
                    shifted_eeg_arrays.append(np.array([]))
                    reduced_fnirs_arrays.append(np.array([]))

        subjects_eeg_list.append(shifted_eeg_arrays)
        subjects_fnirs_list.append(reduced_fnirs_arrays)

    concatenated_eeg_delays = []
    concatenated_fnirs_delays = []

    for subject_eeg, subject_fnirs in zip(subjects_eeg_list, subjects_fnirs_list):
        
        for delay_eeg, delay_fnirs in zip(subject_eeg, subject_fnirs):
            normalized_eeg = (delay_eeg - np.mean(delay_eeg))/ np.std(delay_eeg)
            normalized_fnirs = (delay_fnirs - np.mean(delay_fnirs))/ np.std(delay_fnirs)
            #print("This is the shape of normalized_eeg", normalized_eeg.shape)
            #print("This is the shape of normalized_fnirs", normalized_fnirs.shape)
            concatenated_eeg_delays.append(normalized_eeg)
            concatenated_fnirs_delays.append(normalized_fnirs)

    # Assuming concatenated_eeg_corr is already defined
    concatenated_eeg_results = []
    num_iterations_eeg = np.arange(int(len(concatenated_eeg_delays) / num_shifts))
    ...
    for i in range(num_shifts):
        ...
        for j in num_iterations_eeg:
            ...
            if index < len(concatenated_eeg_delays):
                concatenated_list_eeg.append(concatenated_eeg_delays[index])
    ...
    num_iterations_fnirs = np.arange(int(len(concatenated_fnirs_delays) / num_shifts))
    ...
    for i in range(num_shifts):
        ...
        for j in num_iterations_fnirs:
            index = i + j * num_shifts
            if index < len(concatenated_fnirs_delays):
                concatenated_list_fnirs.append(concatenated_fnirs_delays[index])
        # Check if concatenated_list is not empty and concatenate its elements
    ...
    return concatenated_eeg_results, concatenated_fnirs_results
...
def correlation(theta_power, hbo_concentration):
    ...
    for eeg, hbo in zip(theta_power, hbo_concentration):
        print("this is the shape of eeg", eeg.shape)
        correlation_coefficient, p_value = pearsonr(eeg, hbo)
        correlation_coef.append(correlation_coefficient)
...
if __name__ == "__main__":
    def main():
        ...
        for start in range(iterations):
            concat_array_hc_theta = np.concatenate(flattened_nback_hc_theta[start::interval])
            concat_array_p_theta = np.concatenate(flattened_nback_p_theta[start::interval])
            concat_array_hc_hbo = np.concatenate(flattened_nback_hc_hbo[start::interval])
            concat_array_p_hbo = np.concatenate(flattened_nback_p_hbo[start::interval])
        ...
        print("this is the length of result_hc_theta", len(result_hc_hbo))

        for delay in result_hc_theta:
