...
def compute_and_plot_correlations(subjects_eeg, subjects_fnirs, num_shifts, title_suffix):
    ...
    subj_pvalue_results = []
    for corr_eeg, corr_fnirs in zip(correlation_eeg_subj_theta, correlation_fnirs_subj):
        correlation_results = []
        pvalue_results = []
    ...
    for index_delay, eeg_subject_theta, eeg_subject_delta, fnirs_subject in zip(index_min_pval, correlation_eeg_subj_theta, correlation_eeg_subj_delta, correlation_fnirs_subj):
        eeg_theta =  eeg_subject_theta[index_delay]
        #eeg_delta = eeg_subject_delta[index_delay]
        fnirs_features = fnirs_subject[index_delay]
        X = np.column_stack((eeg_theta, fnirs_features))
    ...
    return (max_corr_values, min_pvalues)

# Compute and plot correlations for healthy controls
healthy_corr = compute_and_plot_correlations(hc_subj_shifted_EEG_arrays, hc_subj_reduced_fnirs_arrays, num_shifts, "Healthy Controls")

healthy_max_corr = healthy_corr[0]
array_healthy_max_corr = np.array(healthy_max_corr)
healthy_min_pval = healthy_corr[1]
array_healthy_min_pval = np.array(healthy_min_pval)

# Compute and plot correlations for patients
patient_corr = compute_and_plot_correlations(p_subj_shifted_EEG_arrays, p_subj_reduced_fnirs_arrays, num_shifts, "Patients")

patient_max_corr = patient_corr[0]
array_patient_max_corr = np.array(patient_max_corr)
patient_min_pval = patient_corr[1]
array_patient_min_pval = np.array(patient_min_pval)

correlations = [array_healthy_max_corr, array_patient_max_corr]
pval = [array_healthy_min_pval, array_patient_min_pval]
labels = ["HC", "Patients"]
colors = ['peachpuff', 'orange']

fig, ax = plt.subplots()
ax.set_ylabel('Maximum Correlation Values')

bplot = ax.boxplot(correlations,
...
fig, ax = plt.subplots()
ax.set_ylabel('Minimum Pvalues Values')

bplot = ax.boxplot(pval,
                   patch_artist=True)  # will be used to label x-ticks
