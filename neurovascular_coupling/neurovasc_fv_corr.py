...
def compute_and_plot_correlations(eeg_delays, fnirs_delays, delay_time, title_suffix):
    ...
    for eeg_array, fnirs_array in zip(correlation_eeg, correlation_fnirs):
        subject_correlations = []
        subject_p_values = []

        for subject_idx in range(eeg_array.shape[0]):
            features_eeg = eeg_array[subject_idx, :]
            features_fnirs = fnirs_array[subject_idx, :]
    ...
    print("This is the shape of correlation_results", correlation_results.shape)
    p_values = np.array(p_values)
    print("This is the shape of pvalues", p_values.shape)

    plt.figure(figsize=(14, 6))

    plt.subplot(2, 1, 1)
    for subject_idx in range(correlation_results.shape[1]):
        plt.plot(delay_time, correlation_results[:, subject_idx], label=f"Subject {subject_idx + 1}")
    ...
    for subject_idx in range(p_values.shape[1]):
        plt.plot(delay_time, p_values[:, subject_idx], label=f"Subject {subject_idx + 1}")
    plt.xlabel("Time Delay")
    plt.ylabel("p-value")
