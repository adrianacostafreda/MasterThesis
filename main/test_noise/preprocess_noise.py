...
def epochs_from_raw(raw, raw_0back, raw_1back, raw_2back, raw_3back, raw_baseline_0back, raw_baseline_1back, raw_baseline_2back, raw_baseline_3back):
    ...
    epochs_0back = mne.Epochs(raw_0back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_1back = mne.Epochs(raw_1back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_2back = mne.Epochs(raw_2back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_3back = mne.Epochs(raw_3back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_baseline_0back = mne.Epochs(raw_baseline_0back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_baseline_1back = mne.Epochs(raw_baseline_1back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_baseline_2back = mne.Epochs(raw_baseline_2back, events, tmin=-0.2, tmax=tstep,preload=True)
    epochs_baseline_3back = mne.Epochs(raw_baseline_3back, events, tmin=-0.2, tmax=tstep,preload=True)
...
for file in os.listdir(folder_hc_baseline_eeg):
    ...
    if filename.endswith(".xdf"):
        ...
        raw.compute_psd(fmin=1, fmax=100).plot(average=True, picks="eeg", exclude="bads")
        plt.show()
        ...
        time = epoch.times[-1] #time in seconds 
        print("This is the time of the epochs", time)
        start_seconds = int(-0.2*sampling_freq)
        final_seconds = int(time*sampling_freq)
        
        print(sampling_freq)

        data = epoch.get_data(units = "uV") # convert from V to uV
        
        epoch.compute_psd(fmin=1, fmax=50).plot(picks="eeg", exclude ="bads")
        plt.show()
        ...
        data = data[:,0,:]
        data = data[-2]
        print("This is the shape of the epochs", data.shape)

        """plt.plot(times, data)
        plt.show()"""

        fft_spectrum = np.fft.rfft(data)
        freq = np.fft.rfftfreq(data.size, d=1/sampling_freq)
        print("This is the length of freq", len(freq))

        fft_spectrum_abs = np.abs(fft_spectrum)
        print("this is the length of fft_spectrum", len(fft_spectrum_abs))
        ...
        plt.plot(freq[:50],fft_spectrum_abs[:50])
        plt.show()
