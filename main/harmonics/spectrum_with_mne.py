...
for file in os.listdir(folder_hc_baseline_eeg):
    ...
    if filename.endswith(".fif"):
        ...
        i_trial_trigger4 = np.where(epochs.events[:, 2] == event_id["Trigger 4"])[0]
        i_trial_trigger5 = np.where(epochs.events[:, 2] == event_id["Trigger 5"])[0]
        i_trial_trigger6 = np.where(epochs.events[:, 2] == event_id["Trigger 6"])[0]
        i_trial_trigger7 = np.where(epochs.events[:, 2] == event_id["Trigger 7"])[0]
        ...
        psds_4 = psds[i_trial_trigger4,:,i_bin_10hz]
        psds_average_4 = psds_4.mean(axis=0)

        psds_5 = psds[i_trial_trigger5,:,i_bin_10hz]
        psds_average_5 = psds_5.mean(axis=0)

        psds_6 = psds[i_trial_trigger6,:,i_bin_10hz]
        psds_average_6 = psds_4.mean(axis=0)

        psds_7 = psds[i_trial_trigger7,:,i_bin_10hz]
        psds_average_7 = psds_4.mean(axis=0)

        # plot PSDS topography
        fig, ax = plt.subplots(1)
        mne.viz.plot_topomap(psds_average_4, epochs.info, axes=ax)
        plt.show()
        fig, ax = plt.subplots(1)
        mne.viz.plot_topomap(psds_average_5, epochs.info, axes=ax)
        plt.show()
        fig, ax = plt.subplots(1)
        mne.viz.plot_topomap(psds_average_6, epochs.info, axes=ax)
        plt.show()
        fig, ax = plt.subplots(1)
        mne.viz.plot_topomap(psds_average_7, epochs.info, axes=ax)
        plt.show()
