...
for file in os.listdir(folder_hc_baseline_eeg):
    ...
    if filename.endswith(".fif"):
        ...
        sigma_power = bp[0].sum(axis=-2)[3,:]
        sigma_power /= event_count
        features.append(sigma_power)

        beta_power = bp[0].sum(axis=-2)[4,:]
...
df = pd.DataFrame(feature_list, columns =['patient_id', 'n-back', 'baseline', 'delta', 'theta', 'alpha', 'sigma', 'beta', 'se', 'pe', 'zc'])
