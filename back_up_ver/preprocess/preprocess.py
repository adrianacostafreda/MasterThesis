...
def filter_eeg(raw: mne.io.Raw) -> None:
    """
    Preprocess EEG data: filter (bandpass filter between 0.1 and 70 Hz) and a notch filter at 50 Hz
    """
...
for file in os.listdir(folder_hc_baseline_eeg):
    ...
    if filename.endswith(".xdf"):
        ...
        """Cannot be used if we split the data into epochs corresponding to the 4 tests"""
        ...
        #ar = AutoReject(n_interpolates, consensus_percs, picks=["eeg"],
