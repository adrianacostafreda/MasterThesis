...
# ========== Functions ==========
def plot_correlation(df_psd_comparison, band, comparison_cond, region, state, labels,
    ...
    Parameters
    ----------
    df_psd_comparison: A Pandas dataframe of PSD values and clinical outcomes compared between timepoints
    band: The frequency band of the PSD values (e.g. 'Alpha')
    comparison_cond: A list of strings for conditions to compare (e.g. ['0-Back','1-Back', '2-Back, '3-Back'])
    region: A string for the region/channel in interest (e.g. 'Fp1')
    ---clinical_outcome: A string for the clinical outcome in interest ('Accuracy of the N-Back')
    state: A list of strings for the states of interest (e.g. ['N-back','Baseline'])
    labels: A list of strings for x and y labels (e.g. ['Theta PSD at occipital region'])
    ...
    data = df_psd_comparison[df_psd_comparison['Frequency band']==band]
    ...
    if correlation == 'Spearman':
        if len(state) == 1:
            ...
            r[0],p[0] = stats.spearmanr(data[data['State']==state[0]], data[data['State']==state[0]][region]) 
        elif len(state) == 2:
            r[0], p[0] = stats.spearmanr(data[data['State']==state[0]], data[data['State']==state[0]][region])
            r[1], p[1] = stats.spearmanr(data[data['State']==state[1]], data[data['State']==state[1]][region])
        else:
    ...
    if correlation == 'Pearson':
        if len(state) == 1:
            data = data[data['State']==state[0]]
            r[0],p[0] = stats.pearsonr(data[data['State']==state[0]], data[data['State']==state[0]][region]) 
        elif len(state) == 2:
            r[0], p[0] = stats.pearsonr(data[data['State']==state[0]], data[data['State']==state[0]][region])
            r[1], p[1] = stats.pearsonr(data[data['State']==state[1]], data[data['State']==state[1]][region])
        else:
            print('Can only plot 1 or 2 states.')
    ...
    if export == True:
        try:
            ...
            pass
        plt.savefig('Results\{}corscatter_{}_{}_{}_{}.tiff'.format(correlation, region, band, comparison_cond, state),dpi=300,bbox_inches='tight')